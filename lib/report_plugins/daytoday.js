  ...
  function drawChart (day, data, options) {
    ...
    if (options.insulindistribution) {
      ...
      var glookoDailyTotals = data.treatments.filter(function(t) {
        return t.eventType === 'Pump Basal Units' && moment(t.created_at || t.timestamp).isSame(from, 'day');
      });
      if (glookoDailyTotals.length > 0) {
        var glookoTotal = glookoDailyTotals[glookoDailyTotals.length - 1];
        if (glookoTotal.basal !== undefined && glookoTotal.basal !== null) {
          totalBasalInsulin = Number(glookoTotal.basal);
        }
      }
