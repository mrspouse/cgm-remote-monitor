...
function boot (env, language) {
  ...
  function setupInternals (ctx, next) {
    ...
    if (env.settings.isEnabled('alexa') || env.settings.isEnabled('google_actions')) {
    ...
    if (env.settings.isEnabled('google_actions')) {
      ctx.google_actions = require('../plugins/google_actions')(env, ctx);
    }
